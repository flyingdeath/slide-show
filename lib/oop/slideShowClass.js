  ...
  slideShowClass.prototype.initialize = function(){
    if(this.containerId){
      var c = new canvasClass({'containerId':this.containerId, width:this.width, height:this.height});
       c.initifillImage('#',this.fade, this.fadeTime);
    }else{
      var c = 0;
    }
    ...
    if(this.orderSet){
      ...
      for(i in set){
           this.h.createAppendDomObject(image_cache, 'img',{'src': this.imagePrefix+set[i]+'.'+ this.imageExt});
      }
      image_cache = this.h.deleteDomElement(image_cache);
    }else{
